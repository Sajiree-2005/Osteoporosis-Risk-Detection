...
const Result: React.FC = () => {
  ...
  const downloadPDF = async () => {
    ...
    try {
      ...
      pdf.text(`Report ID: #${reportId.current}`, W - margin, 12, {
        align: "right",
      });
      ...
      [
        {
          label: "PATIENT",
          line1: patient?.name || patientId || "—",
          line2: `${patient?.age || "—"} months · ${patient?.gender || "—"}`,
        },
        {
          label: "SCREENING DATE",
          line1: new Date(prediction?.timestamp).toLocaleDateString(),
          line2: "X-ray AI Analysis",
        },
        {
          label: "AI CONFIDENCE",
          line1: `${confidence}%`,
          line2: "EfficientNetV2S + Fusion",
        },
      ].forEach((card, i) => {
      ...
      pdf.setDrawColor(...risk.pdfColor);
      pdf.setLineWidth(0.5);
      pdf.roundedRect(margin, y, contentW, 38, 4, 4, "FD");
      pdf.setFontSize(22);
      pdf.setFont("helvetica", "bold");
      pdf.setTextColor(...risk.pdfColor);
      ...
      pdf.text(pdf.splitTextToSize(riskDesc, contentW - 20), W / 2, y + 22, {
        align: "center",
      });
      // Bar
      ...
      pdf.roundedRect(barX, barY, barW2, 3, 1.5, 1.5, "F");
      pdf.setFillColor(...risk.pdfColor);
      pdf.roundedRect(
        barX,
        barY,
        barW2 * (prediction?.probability || 0),
        3,
        1.5,
        1.5,
        "F",
      ...
      if (Object.keys(allProbs).length > 0) {
        ...
        pdf.setFont("helvetica", "bold");
        pdf.setTextColor(15, 23, 42);
        pdf.text("Class Probabilities", margin, y);
        y += 6;
        const probColors: Record<string, [number, number, number]> = {
          Normal: [16, 185, 129],
          Osteopenia: [245, 158, 11],
          Osteoporosis: [244, 63, 94],
        };
        Object.entries(allProbs).forEach(([label, prob]: [string, any]) => {
          ...
          pdf.setTextColor(15, 23, 42);
          pdf.text(`${(prob * 100).toFixed(1)}%`, W - margin - 3, y + 6.5, {
            align: "right",
          });
          const bx = margin + 50,
            bw = contentW - 70;
          pdf.setFillColor(226, 232, 240);
          pdf.roundedRect(bx, y + 3, bw, 3.5, 1.5, 1.5, "F");
      ...
      pdf.setFont("helvetica", "bold");
      pdf.setTextColor(15, 23, 42);
      pdf.text("Clinical Observations", margin, y);
      y += 6;
      [
      ...
      if (imgSrc) {
        ...
        try {
          ...
          if (gradcamSrc) {
            ...
            pdf.setTextColor(148, 163, 184);
            pdf.text(
              "Red/yellow regions indicate highest model attention areas",
              W / 2,
              y,
              { align: "center" },
            );
          ...
          pdf.setTextColor(148, 163, 184);
          pdf.text(
            "[Image could not be embedded — view online]",
            W / 2,
            y + 10,
            { align: "center" },
          );
      ...
      pdf.setTextColor(148, 163, 184);
      pdf.text(
        "Generated by OsteoAI — Osteoporosis Risk Prediction System",
        W / 2,
        290,
        { align: "center" },
      );
    ...
    <div className="p-6 md:p-10 pt-24 min-h-screen bg-slate-50">
      <div className="max-w-5xl mx-auto">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
          <div>
            ...
            <p className="text-slate-500">
              Comprehensive AI analysis for Patient {patientId}
            </p>
          </div>
        ...
        <div
          id="osteoai-report"
          ref={reportRef}
          className="bg-white p-10 rounded-[2rem] shadow-xl border border-slate-100 mb-12"
        >
          {/* Header */}
          ...
          >
            ...
            <p className="text-slate-600 max-w-lg">
              AI model detected patterns suggesting a{" "}
              <strong>{prediction?.result?.toLowerCase()}</strong> risk level
              for osteoporosis.
            </p>
            <div className="w-full max-w-md mt-6 bg-white/60 rounded-full h-3 overflow-hidden">
              <div
                ...
                style={{
                  width: `${(prediction?.probability || 0) * 100}%`,
                  backgroundColor: risk.colorHex,
                }}
              />
            </div>
          </div>
          ...
          <div className="p-7 bg-slate-900 rounded-2xl text-white flex items-start gap-5">
            ...
            <div>
              ...
              <p className="text-slate-400 text-sm leading-relaxed">
                This report is generated by an AI system and should be used as a
                screening tool only. Final diagnosis must be confirmed by a
